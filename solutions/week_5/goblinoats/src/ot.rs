...
pub fn sender_round_1() -> (BigUint, BigUint, BigUint, BigUint) {
    let p = get_p();
    let g = get_g();
    let mut rng = OsRng;
    // Sender random value
...
pub fn receiver_round_1(si: u32, p: &BigUint, g: &BigUint, v: &BigUint) -> (BigUint, u32, BigUint) {
    let mut rng = OsRng;
    // Receiver random value
    let alpha = rng.gen_biguint_range(&BigUint::one(), &(p - 1u32)); // a ∈ Z_p
    // Receiver index i (e.g., i = 2)
    let i = si % 5;

    // Receiver calculates u
...
pub fn sender_round_2(v: BigUint, beta: BigUint, u: BigUint) -> (BigUint, Vec<Vec<u8>>) {
    let p = get_p();
    let g = get_g();
    // Parameters
...
// Function to encrypt m_j using k_j as the key
pub fn encrypt_aes(k_j: &BigUint, m_j: &Vec<u8>) -> Vec<u8> {
...
pub fn decrypt_aes(k_j: &BigUint, encrypted_data: &[u8]) -> Result<Vec<u8>, aes_gcm::Error> {
    let mut key_bytes = k_j.to_bytes_be();
...
pub fn compute_hash(v: &BigUint, w_j: &BigUint) -> BigUint {
...
pub fn receiver_round_2(v: &BigUint, alpha: &BigUint, p: &BigUint, i: u32, c_j: &Vec<Vec<u8>>) -> String {
